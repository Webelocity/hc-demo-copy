...
const CategoryItem: React.FC<CategoryItemProps> = ({ category, level, selectedSubCat, setSelectedSubCat, parentIsSelected = false, filterQuery = "" }) => {
    ...
    useEffect(() => {
        const checkIfDescendantActive = (cat: Category | Subcategory | ChildSubCategory): boolean => {
    ...
    const renderChildCategories = () => {
        ...
        const childrenParentSelected = isActive || isSubcatActive || parentIsSelected;

        if (isCategory(category)) {
            // Filter subcategories based on search query
            const filteredSubcats = filterQuery
                ? category.categorySubCategories.filter(subcat => categoryMatchesQuery(subcat, filterQuery))
            ...
            const filteredChildren = filterQuery
                ? category.childSubCategories.filter(child => categoryMatchesQuery(child, filterQuery))
    ...
    const getIndentClass = () => {
        switch (level) {
            case 0: return '';
            case 1: return 'ml-[0.625rem]';
            case 2: return 'ml-[1rem]';
            case 3: return 'ml-[1.4rem]';
            case 4: return 'ml-[2.5rem]';
            default: return 'ml-[3.125rem]';
        }
    };
    ...
    return (
        <div className="w-full my-[0.375rem]">
            <div className="flex items-start w-full cursor-pointer transition-all duration-300">
                ...
                {!hasChildren && level > 0 && (
                    <span className={`block mr-[0.625rem] w-[1.25rem] flex-shrink-0 ${getIndentClass()}`} />
                )}
