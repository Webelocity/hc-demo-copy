    ...
    <div className="baseContainer py-[3rem]">
      <div className="bg-[var(--Secondary-100)] rounded-[var(--Radius-md)] p-[1.5rem] sm:p-[2rem] md:p-[2.5rem]">
        <div className="flex flex-col gap-[1.5rem] w-full">
          ...
          {/* Tab Bar */}
          <div className="bg-[var(--Secondary-50)] p-[0.75rem] rounded-[var(--Radius-md)] flex items-center lg:justify-center overflow-x-auto gap-[0.5rem]">
            {tabs.map((tab) => (
              <button
                key={tab}
                onClick={() => setActiveTab(tab)}
                className={`
                                    min-w-fit lg:flex-1 whitespace-nowrap flex gap-[0.5rem] items-center justify-center 
